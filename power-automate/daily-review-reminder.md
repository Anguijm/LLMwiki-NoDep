...
- If a card's question or answer text happens to contain the literal string `next_review:`, the heuristic may produce a false count. This is unlikely in practice but documented for completeness.
- The string search is **case-sensitive**. A card with `Next_Review:` or `NEXT_REVIEW:` would be silently skipped. All cards generated by the `flashcards.md` prompt template use lowercase `next_review:` — but hand-written cards must match this exact casing.
- The expected date format is exactly `YYYY-MM-DD` (e.g., `2026-04-22`). Other formats (`MM/DD/YYYY`, `April 22, 2026`) will not be matched.
- Power Automate operates on SharePoint's server-side file state, which may lag a few minutes behind the user's local sync client. A review session completed moments before the daily flow runs may not yet be reflected server-side, resulting in a stale due-card count.
