...
- **File-layout contract.** What is a note? (`*.md` under a tier folder with required frontmatter.) What is metadata? (`_index.md`, `/srs/<id>.yaml` one file per card, `/_prompts/*.md`.) What is ephemeral, and what is load-bearing? The shape of the folder IS the shape of the app. New files need an explicit bucket.
...
- **Per-card YAML schema stability for `/srs/*.yaml`.** Required fields per `docs/data_schemas.md` — `id`, `question`, `answer`, `ease`, `interval`, `next_review`, `tier`. Optional fields — `last_reviewed`, `source_note`. Required-field semantics are load-bearing — the SM-2 scheduler, the review UI, and any Power Automate flow depend on exact names. Required-field removals or semantic changes are architectural changes (require council review + migration plan); optional fields may be added or removed without a schema-level rollback. One file per card; never consolidate into a bulk `srs.csv`/`srs.yaml` — that's the architecture decision SharePoint-sync concurrency forces.
...
4. If this change touches `/srs/*.yaml`: are required-field removals or semantic changes being proposed? If yes, is there a migration plan for existing cards? Optional-field additions are backward-compatible; optional-field removals are backward-compatible too (readers already tolerate the omitted key).
...
8. Is there a test seam? Can the parser, the per-card YAML reader/writer, the wiki-link resolver, and the SM-2 scheduler be exercised without touching a real filesystem?
9. Does this change assume a runtime dep? (It shouldn't — call it out if it does.) Does it assume network access? (Also shouldn't.)
10. What's the rollback plan if this lands and silently corrupts notes or `/srs/*.yaml` cards? How does the user notice, and how do they recover?
...
- Removing or repurposing a required field from the `/srs/*.yaml` schema, or consolidating per-card YAML files into a bulk `srs.csv`/`srs.yaml`.
